...
export default function App() {
  ...
  function handleRemoveWatchedMovie(idToRemove) {
    setWatched((watched) =>
      watched.filter((movie) => movie.imdbID !== idToRemove)
    );
  }
    ...
    <>
      ...
      <Main>
        <Box>
          {isLoading && <Loader />}
          {!isLoading && !error && (
            <MovieList movies={movies} onSetSelectedId={handleSelectedId} />
          )}
...
function MovieList({ movies, onSetSelectedId }) {
  return (
...
function Movie({ movie, onSetSelectedId }) {
  return (
    <li key={movie.imdbID} onClick={(e) => onSetSelectedId(movie.imdbID)}>
